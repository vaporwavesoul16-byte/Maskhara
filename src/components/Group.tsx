...
import { GroupData, GROUP_COLORS } from '../types';
...
export function Group({ group, isSelected, onSelect, onStartMove, onStartResize, onUpdate, onDelete }: Props) {
  const [editingLabel, setEditingLabel] = useState(false);
  const hex = group.color;
  const bg = `${hex}10`;
  const border = isSelected ? `${hex}90` : `${hex}40`;
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background: '#141414', border: `1px solid ${hex}50`,
      ...
      >
        ...
        {editingLabel
          ...
          : <span onDoubleClick={e => { e.stopPropagation(); setEditingLabel(true); }}
              style={{ fontSize: 11, fontWeight: 600, color: hex, fontFamily: 'Syne, sans-serif', cursor: 'text', whiteSpace: 'nowrap' }}>
              {group.label}
            </span>
        }
