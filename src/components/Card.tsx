...
interface Props {
  ...
  onUpdateImmediate: (patch: Partial<CardData>) => void; // for non-text changes
  onDelete: () => void;
}
...
const HANDLES = [
  { edge: 'nw', style: { top: -4, left: -4 } },
  { edge: 'n',  style: { top: -4, left: '50%', transform: 'translateX(-50%)' } },
  { edge: 'ne', style: { top: -4, right: -4 } },
  { edge: 'e',  style: { top: '50%', right: -4, transform: 'translateY(-50%)' } },
  { edge: 'se', style: { bottom: -4, right: -4 } },
  { edge: 's',  style: { bottom: -4, left: '50%', transform: 'translateX(-50%)' } },
  { edge: 'sw', style: { bottom: -4, left: -4 } },
  { edge: 'w',  style: { top: '50%', left: -4, transform: 'translateY(-50%)' } },
];
...
// ── Image ──────────────────────────────────────────────────
function ImageCard({ card, onUpdateImmediate }: { card: CardData; onUpdateImmediate: (p: Partial<CardData>) => void }) {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <div onMouseDown={e => e.stopPropagation()}
        ...
        style={{ flex: 1, borderRadius: 6, border: card.content ? 'none' : '1.5px dashed rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', cursor: 'pointer', background: card.content ? 'transparent' : 'rgba(255,255,255,0.02)', minHeight: 60 }}>
        {card.content
          ...
          : <div style={{ textAlign: 'center', color: 'rgba(255,255,255,0.18)', fontSize: 12, fontFamily: F }}>
              <ImageIcon size={22} style={{ marginBottom: 5, opacity: 0.4 }} /><div>Click or drop image</div>
            </div>
        }
      </div>
...
// ── Swatch ─────────────────────────────────────────────────
function SwatchCard({ card, onUpdate, onUpdateImmediate }: { card: CardData; onUpdate: (p: Partial<CardData>) => void; onUpdateImmediate: (p: Partial<CardData>) => void }) {
  const swatches: SwatchEntry[] = card.swatches ?? [];
  const updLabel = (idx: number, label: string) => onUpdate({ swatches: swatches.map((s, i) => i === idx ? { ...s, label } : s) });
  const updHex   = (idx: number, hex: string)   => onUpdateImmediate({ swatches: swatches.map((s, i) => i === idx ? { ...s, hex } : s) });
  return (
    <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, alignContent: 'start' }}>
      {swatches.map((sw, i) => (
        <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          <div style={{ height: 32, borderRadius: 5, background: sw.hex || 'rgba(255,255,255,0.06)', border: '1.5px dashed rgba(255,255,255,0.1)', position: 'relative', overflow: 'hidden', cursor: 'pointer' }}>
            <input type="color" value={sw.hex || '#000000'} onChange={e => updHex(i, e.target.value)}
              onMouseDown={e => e.stopPropagation()} onClick={e => e.stopPropagation()}
              style={{ position: 'absolute', inset: 0, opacity: 0, width: '100%', height: '100%', cursor: 'pointer', border: 'none' }} />
          </div>
          <input value={sw.hex} onChange={e => updHex(i, e.target.value)}
            onMouseDown={e => e.stopPropagation()} onClick={e => e.stopPropagation()}
            placeholder="#hex"
          ...
          <input value={sw.label} onChange={e => updLabel(i, e.target.value)}
            onMouseDown={e => e.stopPropagation()} onClick={e => e.stopPropagation()}
            placeholder="Name"
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10, overflowY: 'auto' }}>
      {([
        ...
        <div key={key}>
          ...
          <EditableText value={f[key]} onChange={v => upd(key, v)} placeholder={ph} multiline style={fld} />
        </div>
      ))}
    </div>
  );
...
// ── Project ────────────────────────────────────────────────
function ProjectCard({ card, onUpdate, onUpdateImmediate }: { card: CardData; onUpdate: (p: Partial<CardData>) => void; onUpdateImmediate: (p: Partial<CardData>) => void }) {
  ...
  const updStatus = (status: typeof f.status) => onUpdateImmediate({ projectFields: { ...f, status } });
  const lbl: React.CSSProperties = { fontSize: 9, color: 'rgba(255,255,255,0.25)', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'Syne, sans-serif', marginBottom: 3 };
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10, overflowY: 'auto' }}>
      <div>
      ...
      <div>
        ...
        <EditableText value={f.description} onChange={v => updText('description', v)} placeholder="Describe the project..." multiline
          style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)', fontFamily: F, lineHeight: 1.5 }} />
      </div>
      <div>
        <div style={lbl}>Notes</div>
        <EditableText value={f.notes} onChange={v => updText('notes', v)} placeholder="Timeline, links..." multiline
...
// ── Main ───────────────────────────────────────────────────
export function Card({ card, isSelected, onSelect, onStartMove, onStartResize, onUpdate, onUpdateImmediate, onDelete }: Props) {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, padding: 8, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
        ...
        {card.type === 'image'   && <ImageCard   card={card} onUpdateImmediate={onUpdateImmediate} />}
        {card.type === 'link'    && <LinkCard    card={card} onUpdate={onUpdate} />}
        {card.type === 'swatch'  && <SwatchCard  card={card} onUpdate={onUpdate} onUpdateImmediate={onUpdateImmediate} />}
        {card.type === 'persona' && <PersonaCard card={card} onUpdate={onUpdate} />}
        {card.type === 'project' && <ProjectCard card={card} onUpdate={onUpdate} onUpdateImmediate={onUpdateImmediate} />}
      </div>
